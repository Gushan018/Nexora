...
const updateSystemSettings = async (req, res) => {
  try {
    ...
    const dataPayload = {
      platformName: platformName !== undefined ? platformName : (existing?.platformName || 'Nexora Marketplace'),
      supportEmail: supportEmail !== undefined ? supportEmail : (existing?.supportEmail || 'support@nexora.com'),
      logoUrl: logoUrl !== undefined ? logoUrl : existing?.logoUrl,
      defaultTimezone: defaultTimezone !== undefined ? defaultTimezone : (existing?.defaultTimezone || 'UTC'),
      defaultCurrency: defaultCurrency !== undefined ? defaultCurrency : (existing?.defaultCurrency || 'LKR'),
      maintenanceMode: maintenanceMode !== undefined ? Boolean(maintenanceMode) : (existing?.maintenanceMode || false),
      commissionPercent: commissionPercent !== undefined ? parseInt(commissionPercent) || 0 : (existing?.commissionPercent || 10),
      paymentGateway: paymentGateway !== undefined ? paymentGateway : (existing?.paymentGateway || 'Stripe'),
      smtpHost: smtpHost !== undefined ? smtpHost : existing?.smtpHost,
      smtpPort: smtpPort ? parseInt(smtpPort) : existing?.smtpPort,
      smtpUser: smtpUser !== undefined ? smtpUser : existing?.smtpUser,
      smtpPassword: smtpPassword !== undefined ? smtpPassword : existing?.smtpPassword,
      smtpFromEmail: smtpFromEmail !== undefined ? smtpFromEmail : existing?.smtpFromEmail,
      smtpSecure: smtpSecure !== undefined ? Boolean(smtpSecure) : (existing?.smtpSecure || false),
      authLockoutEnabled: authLockoutEnabled !== undefined ? Boolean(authLockoutEnabled) : (existing?.authLockoutEnabled || false),
      authFailedAttemptsLimit: authFailedAttemptsLimit !== undefined ? parseInt(authFailedAttemptsLimit) : (existing?.authFailedAttemptsLimit || 5),
      authPasswordMinLength: authPasswordMinLength !== undefined ? parseInt(authPasswordMinLength) : (existing?.authPasswordMinLength || 8),
      authTwoFactorEnabled: authTwoFactorEnabled !== undefined ? Boolean(authTwoFactorEnabled) : (existing?.authTwoFactorEnabled || false),
    };
    ...
    res.status(200).json({ message: 'Settings saved successfully', settings: updated });
  } catch (error) {

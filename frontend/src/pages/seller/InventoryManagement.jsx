      ...
      <Card>
        ...
        <div className="p-4 border-b border-slate-200 dark:border-white/10 flex flex-col md:flex-row gap-4 justify-between items-center">
          ...
          <div className="flex flex-wrap sm:flex-nowrap gap-3 w-full md:w-auto">
            <select
              ...
              className="bg-light-surface dark:bg-surface/50 border border-slate-300 dark:border-white/10 rounded-xl px-3 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-primary/50 cursor-pointer"
            >
              <option value="ALL" style={{ color: '#0f172a', backgroundColor: '#ffffff' }}>All Categories</option>
              {categories.map(cat => (
                <option key={cat} value={cat} style={{ color: '#0f172a', backgroundColor: '#ffffff' }}>{cat}</option>
              ))}
            </select>
            ...
            <select
              ...
              className="bg-light-surface dark:bg-surface/50 border border-slate-300 dark:border-white/10 rounded-xl px-3 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-primary/50 cursor-pointer"
            >
              <option value="ALL" style={{ color: '#0f172a', backgroundColor: '#ffffff' }}>All Stock Statuses</option>
              <option value="IN_STOCK" style={{ color: '#0f172a', backgroundColor: '#ffffff' }}>In Stock ({'>'}10)</option>
              <option value="LOW_STOCK" style={{ color: '#0f172a', backgroundColor: '#ffffff' }}>Low Stock (1-10)</option>
              <option value="OUT_OF_STOCK" style={{ color: '#0f172a', backgroundColor: '#ffffff' }}>Out of Stock (0)</option>
            </select>
          </div>
        </div>
        ...
        {isLoading ? (
          <div className="p-12 text-center text-slate-500 flex items-center justify-center gap-2">
            <Loader2 className="w-5 h-5 animate-spin text-primary" /> Loading inventory...
          </div>
                      ...
                      <td className="p-4">
                        <div className="flex items-center justify-center gap-2">
                          <button
                            ...
                            className="p-1 rounded-md border border-slate-300 dark:border-white/10 bg-slate-100 dark:bg-white/5 text-slate-700 dark:text-white hover:bg-red-500 hover:text-white transition-colors disabled:opacity-40"
                          >
                          ...
                          <input
                            ...
                            className="w-16 text-center font-bold bg-light-surface dark:bg-surface border border-slate-300 dark:border-white/10 rounded-lg py-1 text-slate-900 dark:text-white focus:outline-none focus:border-primary text-xs"
                          />
                          <button
                            type="button"
                            onClick={() => handleStockChange(prod.productId, prod.quantity, 1)}
                            disabled={updatingId === prod.productId}
                            className="p-1 rounded-md border border-slate-300 dark:border-white/10 bg-slate-100 dark:bg-white/5 text-slate-700 dark:text-white hover:bg-green-500 hover:text-white transition-colors"
                          >
                      ...
                      <td className="p-4 pr-6 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <Link to={`/seller/edit-product/${prod.productId}`}>
                            <button className="p-1.5 rounded-lg border border-slate-300 dark:border-white/10 hover:border-primary text-slate-600 dark:text-slate-300 hover:text-primary transition-colors">
                              <Edit3 className="w-4 h-4" />
                            </button>
                          </Link>
                          <button
                            type="button"
                            onClick={() => deleteProductMutation.mutate(prod.productId)}
                            className="p-1.5 rounded-lg border border-slate-300 dark:border-white/10 hover:border-red-500 text-slate-600 dark:text-slate-300 hover:text-red-500 transition-colors"
                          >

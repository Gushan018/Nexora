    ...
    <div className="w-full relative overflow-hidden bg-background min-h-screen">
      ...
      <section className="relative min-h-[95vh] flex flex-col justify-center items-center pt-24 pb-32">
        ...
        <div className="relative z-20 max-w-7xl mx-auto px-4 w-full flex flex-col items-center text-center mt-10">
          ...
          >
            {[
              { name: 'Verified Vendors', count: `${vendors.length} VENDORS` },
      ...
      <section className="py-24 relative z-10 max-w-7xl mx-auto px-4 border-b border-slate-200">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-serif text-slate-900 mb-4">Curated Event Packages</h2>
          <p className="text-slate-600 max-w-xl mx-auto text-sm leading-relaxed">
            Every package and service is provided by verified top-tier vendors on our platform.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {featuredPackages.map((item, i) => (
            ...
            >
              <img 
                src={item.images?.[0] ? resolveAssetUrl(item.images[0]) : DEFAULT_PACKAGE_IMAGES[i % DEFAULT_PACKAGE_IMAGES.length]} 
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-[#0A101D] via-[#0A101D]/40 to-transparent flex flex-col justify-end p-8">
                ...
                <h3 className="text-2xl font-serif text-slate-900 mb-2">{item.packageName}</h3>
                <p className="text-slate-600 text-sm leading-relaxed line-clamp-2">{item.description}</p>
                <p className="text-primary text-xs font-semibold mt-2">{item.vendor?.businessName || 'Verified Vendor'}</p>
              </div>
            </motion.div>
          ))}
        </div>
      ...
      <section className="relative z-20 max-w-7xl mx-auto px-4 mt-32">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
          {categories.map((cat, i) => (
            ...
            >
              <img 
                src={DEFAULT_CAT_IMAGES[cat.categoryName] || 'https://images.unsplash.com/photo-1511285560929-80b456fea0bc?w=500&q=80'} 
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/30 to-transparent flex items-end p-6">
                <span className="text-slate-900 font-bold text-xl drop-shadow-lg transform translate-y-2 group-hover:translate-y-0 transition-transform">{cat.categoryName}</span>
              </div>
            </motion.div>
          ))}
        </div>
      ...
      <section className="relative z-20 max-w-7xl mx-auto px-4 mt-32">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-serif text-slate-900 mb-4">Loved by Hosts & Clients</h2>
          <p className="text-slate-600 max-w-2xl mx-auto text-lg">Real reviews from event hosts and clients on the Event Nest platform.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {featuredReviews.map((testimonial, i) => (
            <div key={testimonial.reviewId || i} className="bg-surface/50 border border-slate-300 rounded-3xl p-8 backdrop-blur-sm relative">
              ...
              <div className="flex items-center gap-4 mb-6">
                ...
                />
                <div>
                  <h4 className="text-slate-900 font-bold">{testimonial.customer?.name || 'Event Client'}</h4>
                  <p className="text-primary text-sm font-medium">{testimonial.vendor?.businessName ? `Client of ${testimonial.vendor.businessName}` : 'Verified Event Host'}</p>
                </div>
              </div>
              ...
              <p className="text-slate-700 leading-relaxed italic">"{testimonial.comment}"</p>
            </div>
          ))}
        </div>
      </section>

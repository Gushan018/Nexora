...
import { api, resolveProductImageUrl } from '../../utils/api';
      ...
      <div className="container mx-auto px-4 sm:px-6 max-w-7xl">
        ...
        <div className="mb-12">
          ...
          <motion.p 
            ...
            className="text-slate-600 dark:text-slate-300 text-lg max-w-2xl"
          >
...
const ProductCard = ({ product, wishlistItems = [] }) => {
  ...
  return (
    <div 
      className="glass-card rounded-2xl overflow-hidden group cursor-pointer border border-slate-200 dark:border-white/10 bg-white dark:bg-[#1C2333] hover:border-primary/50 transition-colors duration-500 shadow-md"
      ...
      <div className="p-6">
        <div className="flex items-start justify-between mb-2">
          <div>
            ...
            <h3 className="text-lg font-semibold text-slate-900 dark:text-white leading-tight mb-2 group-hover:text-primary transition-colors">
              {product.productName}
            </h3>
          </div>
          <div className="text-right">
            <span className="text-xl font-bold text-slate-900 dark:text-white">LKR {Number(product.price).toFixed(2)}</span>
          </div>
        </div>
        
        <div className="flex items-center gap-2 text-sm">
          <Star className={cn("w-4 h-4", reviewsCount > 0 ? "text-yellow-500 fill-yellow-500" : "text-slate-300")} />
          <span className="text-slate-900 dark:text-white font-medium">{avgRating ? avgRating : 'New'}</span>
          <span className="text-slate-500 dark:text-slate-400">({reviewsCount} {reviewsCount === 1 ? 'review' : 'reviews'})</span>
        </div>
      </div>
    </div>
  );

...
const ROLES = [
  ...
  { id: 'ROL-3', name: 'Moderator', description: 'Responsible for reviewing vendor KYB documents and flagged products.', users: 8, type: 'Custom', permissions: ['View Users', 'Approve Vendors', 'Moderate Products'] },
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-white/60">Configure access levels and permissions for internal team members.</p>
        </div>
                ...
                <tr key={i} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors group">
                  <td className="p-4 pl-6">
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs text-white/50 mt-1 max-w-sm">{role.description}</span>
                    </div>
                  </td>
                  <td className="p-4">
                  ...
                  <td className="p-4">
                    <div className="flex flex-wrap gap-1.5 max-w-[300px]">
                      {role.permissions.map((perm, idx) => (
                        <span key={idx} className="px-2 py-1 bg-surface border border-white/10 rounded-md text-[10px] font-medium text-white/70">
                          {perm}
                        </span>
                      ))}
                      {role.permissions.length > 2 && role.type !== 'System' && (
                        <span className="px-2 py-1 bg-surface border border-white/10 rounded-md text-[10px] font-medium text-white/40">
                          + More
                        </span>
                      )}
                    </div>
                  ...
                  <td className="p-4 pr-6 text-right space-x-2">
                    ...
                    <button className="p-2 text-white/40 hover:text-white hover:bg-white/10 rounded-lg transition-colors" title="Actions" disabled={role.type === 'System'}>
      ...
      <div className="p-4 rounded-xl border border-primary/20 bg-primary/5 flex gap-4 items-start">
        ...
        <div>
          ...
          <p className="text-xs text-white/60 mt-1">Always follow the Principle of Least Privilege (PoLP). Only assign the minimum permissions necessary for a team member to perform their job. System roles cannot be deleted.</p>
        </div>
      </div>

...
const LOGS = [
  ...
  { id: 'EVT-002', actor: 'System Algorithm', action: 'Auto-Approved Vendor', target: 'APP-1045', ip: 'internal-process', date: '15 mins ago', risk: 'Low' },
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-white/60">Immutable record of all administrative and critical system actions.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <Card className="border-white/10">
          <CardContent className="p-6">
            <h3 className="text-sm font-medium text-white/60 mb-2">Events Logged (24h)</h3>
        ...
        <Card className="border-white/10">
          <CardContent className="p-6">
            <h3 className="text-sm font-medium text-white/60 mb-2">Active Admins</h3>
            <div className="flex items-end gap-3">
        ...
        <Card className="border-red-500/20 bg-red-500/5">
          <CardContent className="p-6">
            <h3 className="text-sm font-medium text-white/80 mb-2">High Risk Events</h3>
            <div className="flex items-end gap-3">
        ...
        <Card className="border-white/10">
          <CardContent className="p-6 flex justify-between items-center h-full">
            <div>
              <h3 className="text-sm font-medium text-white/60 mb-2">System Integrity</h3>
              <div className="flex items-end gap-3">
      ...
      <Card>
            ...
            <tbody className="text-sm font-mono">
              {LOGS.map((log, i) => (
                <tr key={i} className={cn(
                  ...
                  log.risk === 'High' ? "bg-red-500/5 hover:bg-red-500/10" : "hover:bg-white/[0.02]"
                )}>
                  <td className="p-4 pl-6">
                    <div className="flex flex-col">
                      <span className="text-white/80">{log.date}</span>
                      <span className="text-xs text-white/40">{log.id}</span>
                    </div>
                  </td>
                  <td className="p-4 text-white font-sans font-medium">{log.actor}</td>
                  <td className="p-4 text-primary">{log.action}</td>
                  <td className="p-4 text-white/60">{log.target}</td>
        ...
        <div className="p-4 border-t border-white/5 bg-surface/30">
          <p className="text-xs text-white/40 font-mono text-center flex items-center justify-center gap-2">

...
export const Withdrawals = () => {
  ...
  const netEarnings = statsData?.netEarnings !== undefined && statsData?.netEarnings !== null
    ? Number(statsData.netEarnings)
    : 12450.00;

  const availableBalance = netEarnings > 0 ? netEarnings : 12450.00;
  ...
  const handleWithdraw = () => {
    ...
    const newTx = {
      ...
      amount: `LKR ${Number(amount).toLocaleString('en-US', { minimumFractionDigits: 2 })}`,
      status: 'Processing',
  ...
  if (isRequested) {
    return (
      <div className="pt-24 pb-20 min-h-[calc(100vh-8rem)] flex flex-col items-center justify-center text-slate-900 dark:text-slate-100">
        <motion.div 
        ...
        >
          ...
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-4">Withdrawal Initiated</h1>
          <p className="text-slate-600 dark:text-slate-300 mb-8">
            Your request for <strong className="text-slate-900 dark:text-white">LKR {Number(amount).toLocaleString()}</strong> is being processed. Funds will transfer to <strong className="text-slate-900 dark:text-white">{bankInfo.bankName}</strong> within 1-3 business days.
          </p>
      ...
      <Modal isOpen={isBankModalOpen} onClose={() => setIsBankModalOpen(false)} title="Update Payout Bank Account">
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-4">
            ...
            <Button onClick={() => setIsBankModalOpen(false)}>Save Account Details</Button>
          </div>
        </div>
      </Modal>
    </div>
